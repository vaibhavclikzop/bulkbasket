...
class Customers
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        if (empty(session('customer_token'))) {
            ...
            $user = DB::table("customer_users as a")->select("a.*")->join("customers as b", "a.customer_id", "b.id")->where('a.token', session('customer_token'))->where("b.active", 1)->first();
            if (empty($user)) {
                session()->flush();
              ...
              // $attendance = DB::table("attendance")->where("emp_id", $user->id)->whereDate("start_time", now())->first();
                ...
                $request->merge(["user" => (array)$user, "userIds" => $child_id]);
